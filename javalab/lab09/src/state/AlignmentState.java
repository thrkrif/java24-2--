...
// Concrete State : move toward the average direction
public class AlignmentState implements BoidState {
    ...
    public void applyBehavior(Boid boid, List<Boid> boids) {
        ...
        if (count > 0) {
            ...
            // SeparationState로 변경 : boid와 other의 거리가 < SEPARATION_DISTANCE 
            for (Boid other : boids) {
                if (other != boid && boid.getPosition().distance(other.getPosition()) < Boid.SEPARATION_DISTANCE) {
                    ...
                    boid.setState(new SeparationState());
                    return;
                }
            }
        // Boid.VISION_RANGE 내에 boid가 없는 경우, 즉, count = 0 인 경우 WanderState로 변경
        } else {
